...
class Env:
    ...
    def __init__(self, server_addr, num_threads=8):
        ...
        self.round = [1 for _ in range(self.num_threads)]
        self.ts = [0 for _ in range(self.num_threads)]
        self.headers = {"accept": "application/json"}
        self.actions = {'usa': [], 'chn': []}
        self.action_index = {}
        self.minmax = {}
        self.init_action()
        self.state_uuid = ["" for _ in range(num_threads)]
    ...
    def reset(self, thread_idx):
        self.round[thread_idx] = 1
        self.ts[thread_idx] = 0
        res = requests.post(f"{self.server_addr}reset?model_name=gtapXP", headers=self.headers)
        data = res.json()
        self.state_uuid[thread_idx] = data['res']['state_uuid']
        return self.formate_state(data['res']['state'])

    def get_available_actions(self, region, thread_idx):
        res = requests.post(f"{self.server_addr}get_action_space", headers=self.headers, params={'rnd': self.round[thread_idx]})
    ...
    def step(self, action, region, thread_idx):
        params = {"state_uuid": self.state_uuid[thread_idx], "action": self.actions[region][action]}
        # print(thread_idx, self.state_uuid[thread_idx], params)
        print(region, thread_idx, ",".join([x['node_desc'] for x in self.actions[region][action]]))
        res = requests.post(f"{self.server_addr}step", headers=self.headers, json=params)
        data = res.json()
        self.state_uuid[thread_idx] = data['res']['next_state_uuid']
        state = self.formate_state(data['res']['next_state'])
        reward = self.formate_reward(data['res']['reward'])

        self.ts[thread_idx] += 1
        if self.ts[thread_idx] > 3:
            done = True
        else:
            done = False
        if self.ts[thread_idx] % 2 == 0:
            self.round[thread_idx] += 1
